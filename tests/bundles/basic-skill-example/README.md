...
testable. Use it to verify skills loading and skill script runners are wired
correctly on whatever environment you are setting up.
...
3. A **stateful-lifetime skill** `basic-skill-example.warm-counter-skill` and
 ...
 one is wired through the platform's `WorkspaceContainerManager`: the first
